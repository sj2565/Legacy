...
// STM32에서 압력 데이터 가져오기
const pressureWebsocket = new WebSocket("ws://localhost:3000"); // 서버 연결
pressureWebsocket.onmessage = (event) => {
	try {
		const data = JSON.parse(event.data);
		if (data.pressure != undefined) {
			sensorPressure = data.pressure; // 최신 압력 값 업데이트
		}
	} catch (error) {
		console.error("압력 데이터 처리 오류 : ", error);
	}
};
